...
export default function RegisterPage() {
  ...
  const cfOk = cf && cf === pw;
  const canSubmit =
    emailValid && username.length >= 3 && pwOk && cfOk && agree && !loading;
        ...
        <div className="max-w-md w-full relative">
          ...
          <div className="relative rounded-3xl border border-red-100/80 bg-white/80 backdrop-blur-xl p-8 shadow-xl shadow-red-500/10">
            ...
            <p className="text-sm text-neutral-600 mb-6 text-center">
              Train smarter. No installs. Just your camera.
            </p>
            ...
            <form onSubmit={handleSubmit} className="space-y-4" noValidate>
              ...
              <div>
                ...
                <input
                  type="email"
                  className={`${inputBase} ${
                    email ? (emailValid ? state.ok : state.bad) : state.neu
                  }`}
              ...
              <div>
                ...
                <input
                  ...
                  className={`${inputBase} ${
                    pw ? (pwOk ? state.ok : state.bad) : state.neu
                  }`}
                  placeholder="••••••••"
                  value={pw}
                ...
                <ul className="mt-2 text-xs space-y-1">
                  {Object.entries(pwState).map(([k, v]) => (
                    <li
                      key={k}
                      className={v ? "text-emerald-600" : "text-neutral-500"}
                    >
                      {v ? "✓" : "•"}{" "}
                      {k === "len"
                        ? "8+ chars"
              ...
              <div>
                ...
                <input
                  type={showPw ? "text" : "password"}
                  className={`${inputBase} ${
                    cf ? (cfOk ? state.ok : state.bad) : state.neu
                  }`}
                  placeholder="••••••••"
                  value={cf}
